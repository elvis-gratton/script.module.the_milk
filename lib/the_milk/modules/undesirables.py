...
from the_milk.modules.control import existsPath, dataPath, makeFile, undesirablescacheFile, lang
...
class Undesirables():
    ...
    def check_database(self):
        ...
        try:
            ...
            self.dbcur.execute(
                'CREATE TABLE IF NOT EXISTS undesirables (keyword TEXT NOT NULL, user_defined BOOL NOT NULL, enabled BOOL NOT NULL, UNIQUE(keyword))')
            self.set_defaults()
            return False

    def set_defaults(self):
        from the_milk.modules.source_utils import UNDESIRABLES
...
def undesirablesSelect():
    from the_milk.modules.control import multiselectDialog
    from the_milk.modules.source_utils import UNDESIRABLES
...
def undesirablesUserRemove():
    from the_milk.modules.control import multiselectDialog, notification
    undesirables_cache = Undesirables()
...
def undesirablesUserRemoveAll():
    from the_milk.modules.control import yesnoDialog, notification
    undesirables_cache = Undesirables()
    user_undesirables = undesirables_cache.get_user_defined()
    if not user_undesirables: return notification(message=32084)
...
def undesirablesInput():
    from the_milk.modules.control import dialog
    undesirables_cache = Undesirables()
    user_defined = undesirables_cache.get_user_defined()
    if user_defined:
        current_user_string = ','.join(user_defined)
    else:
        current_user_string = ''
