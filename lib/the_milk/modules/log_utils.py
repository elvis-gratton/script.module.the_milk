...
import inspect
from the_milk.modules.control import transPath, setting as getSetting, lang, joinPath, existsPath
...
DEBUGPREFIX = '[COLOR red][ the_milk %s ][/COLOR]'
...
def log(msg, caller=None, level=LOGINFO):
    ...
    try:
        ...
        if debug_location == '1':
            log_file = joinPath(LOGPATH, 'the_milk.log')
            ...
            if not reverse_log:
                with open(log_file, 'a', encoding='utf-8') as f:  # "with" auto cleans up and closes
                    line = '[%s %s] %s: %s' % (
                    datetime.now().date(), str(datetime.now().time())[:8], DEBUGPREFIX % debug_list[level], msg)
                    f.write(line.rstrip('\r\n') + '\n')
...
def clear_logFile():
    cleared = False
    try:
        from the_milk.modules.control import yesnoDialog
...
def view_LogFile(name):
    try:
        ...
        from the_milk.modules.control import addonPath
        log_file = joinPath(LOGPATH, '%s.log' % name.lower())
        if not existsPath(log_file):
            from the_milk.modules.control import notification
...
def upload_LogFile():
    ...
    try:
        import requests
        from the_milk.modules.control import addonVersion, selectDialog
        f = open(log_file, 'r', encoding='utf-8', errors='ignore')
        text = f.read()
        f.close()
        UserAgent = 'the_milk %s' % addonVersion()
        ...
        if 'key' in response.json():
            ...
            log('the_milk log file uploaded to: %s' % result)
            ...
            if 'Copy url To Clipboard' in list[select][0]:
                from the_milk.modules.source_utils import copy2clip
                copy2clip(list[select - 1][1])
        elif 'message' in response.json():
            notification(message='the_milk Log upload failed: %s' % str(response.json()['message']))
            log('the_milk Log upload failed: %s' % str(response.json()['message']), level=LOGERROR)
        else:
