# -*- coding: utf-8 -*-
"""
	the_milk Module  Modified 01-02-23 UD
"""
...
addonObject = addon('script.module.the_milk')
...
def refresh_debugReversed():  # called from service "onSettingsChanged" to clear the_milk.log if setting to reverse has been changed
    if homeWindow.getProperty('the_milk.debug.reversed') != setting('debug.reversed'):
        homeWindow.setProperty('the_milk.debug.reversed', setting('debug.reversed'))
        execute('RunPlugin(plugin://script.module.the_milk/?action=tools_clearLogFile)')
...
def isVersionUpdate():
    ...
    try:
        ...
        xbmc.log('the_milk Addon Data Path Does not Exist. Creating Folder....', LOGINFO)
        addon_folder = transPath('special://profile/addon_data/script.module.the_milk')
    ...
    try:
        ...
        oldVersion = '0'
    try:
        curVersion = addon('script.module.the_milk').getAddonInfo('version')
...
def clean_settings():
    ...
    addon_id = 'script.module.the_milk'
    try:
        removed_settings = []
        active_settings = []
