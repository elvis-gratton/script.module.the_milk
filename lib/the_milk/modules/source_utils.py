...
from the_milk.modules import cleantitle
from the_milk.modules.undesirables import Undesirables
from the_milk.modules.control import homeWindow, setting as getSetting, setSetting
...
CAM = (
'1xbet', 'betwin', '.cam.', 'camrip', 'cam.rip', 'dvdcam', 'dvd.cam', 'dvdts', 'hdcam', '.hd.cam', '.hctc', '.hc.tc',
'.hdtc',
'.hd.tc', 'hdts', '.hd.ts', 'hqcam', '.hg.cam', '.tc.', '.tc1', '.tc7', '.ts.', '.ts1', '.ts7', 'tsrip', 'telecine',
'telesync', 'tele.sync')

LANG = (
'arabic', 'bengali', 'bgaudio', 'castellano', 'chinese', 'dutch', 'finnish', 'french', 'german', 'greek', 'indonesian',
'hebrew', 'italian', 'korean', 'latino', 'polish',
'portuguese', 'russian', 'swedish', 'spanish', 'tamil', 'telugu', 'truefrench', 'truespanish', 'turkish')

ABV_LANG = (
'.ara.', '.ces.', '.chi.', '.chs.', '.cze.', '.dan.', '.de.', '.deu.', '.dut.', '.ell.', '.es.', '.esl.', '.esp.',
'.fi.', '.fin.', '.fr.', '.fra.', '.fre.', '.frn.', '.gai.', '.ger.', '.gle.', '.gre.',
'.gtm.', '.he.', '.heb.', '.hi.', '.hin.', '.hun.', '.hindi.', '.ind.', '.iri.', '.it.', '.ita.', '.ja.', '.jap.',
'.jpn.', '.ko.', '.kor.', '.lat.', '.nl.', '.lit.', '.nld.', '.nor.', '.pl.', '.pol.',
'.pt.', '.por.', '.ru.', '.rus.', '.som.', '.spa.', '.sv.', '.sve.', '.swe.', '.tha.', '.tr.', '.tur.', '.uae.', '.uk.',
'.ukr.', '.vi.', '.vie.', '.zh.', '.zho.')
...
UNWANTED_NAMES = [' en torrent', 'extended', '(version longue)', 'version longue', 'amzn']

UNDESIRABLES = ['400p.octopus', '720p.octopus', '1080p.octopus', 'alexfilm', 'amedia', 'audiobook', 'baibako',
                'bigsinema', 'bonus.disc', 'casstudio.tv', 'courage.bambey',
                '.cbr', '.cbz', 'coldfilm', 'dilnix', 'dutchreleaseteam', 'e.book.collection', 'empire.minutemen',
                'eniahd', '.exe', 'exkinoray', 'extras.only',
                'gears.media', 'gearsmedia', 'good.people', 'gostfilm', 'hamsterstudio', 'hdrezka', 'hdtvrip', 'hurtom',
                'idea.film', 'ideafilm', 'jaskier', 'kapatejl6', 'kb.1080p',
                'kb.720p', 'kb.400p', 'kerob', 'kinokopilka', 'kravec', 'kuraj.bambey', 'lakefilm', 'lostfilm',
                'megapeer', 'minutemen.empire', 'newstudio',
                'omskbird', '.ost.', 'paravozik', 'profix.media', 'rifftrax', 'sample', 'soundtrack', 'subtitle.only',
                'sunshinestudio', 'teaser', 'trailer', 'tumbler.studio',
                'tvshows', 'ultradox', 'viruseproject', 'vostfr', 'vo.stfr', 'web.dlrip', 'webdlrip', 'wish666',
                'pa.web.dl', '.p.web.dl', '.d.web.dl', '.dt.web.dl']
...
season_list = (
'one', 'two', 'three', 'four', 'five', 'six', 'seven', 'eigh', 'nine', 'ten', 'eleven', 'twelve', 'thirteen',
'fourteen',
...
season_ordinal2_list = (
'1st', '2nd', '3rd', '4th', '5th', '6th', '7th', '8th', '9th', '10th', '11th', '12th', '13th', '14th', '15th', '16th',
'17th', '18th', '19th', '20th', '21st', '22nd', '23rd', '24th', '25th')

season_dict = {'1': 'one', '2': 'two', '3': 'three', '4': 'four', '5': 'five', '6': 'six', '7': 'seven', '8': 'eigh',
               '9': 'nine', '10': 'ten',
...
season_ordinal_dict = {'1': 'first', '2': 'second', '3': 'third', '4': 'fourth', '5': 'fifth', '6': 'sixth',
...
season_ordinal2_dict = {'1': '1st', '2': '2nd', '3': '3rd', '4': '4th', '5': '5th', '6': '6th', '7': '7th', '8': '8th',
                        '9': '9th', '10': '10th',
                        '11': '11th', '12': '12th', '13': '13th', '14': '14th', '15': '15th', '16': '16th',
                        '17': '17th', '18': '18th', '19': '19th',
...
def check_foreign_audio():
    return False if home_getProperty('fs_filterless_search') == 'true' else getSetting(
...
def check_title(title, aliases, release_title, hdlr, year, years=None):  # non pack file title check, single eps and movies
    if years:  # for movies only, scraper to pass None for episodes
        ...
        if not re.search(r'%s' % hdlr, release_title, re.I):
            return False

    aliases_list = aliases_to_array(aliases)
    title_list = []
    title_list_append = title_list.append
    if aliases_list:
        for i in aliases_list:
            try:
                alias = i.replace('&', 'and').replace(year, '')
                if years:  # for movies only, scraper to pass None for episodes
                    for i in years:
    ...
    try:
        ...
        if all(cleantitle.get(i) != cleantitle_t for i in title_list):
            return False
        ...
        if hdlr != year:  # equal for movies but not for shows
            range_regex = (
                ...
                r'saison[.-]?\d{1,3}[.-]?ep[.-]?\d{1,3}[-.]ep[.-]?\d{1,3}',
                r'season[.-]?\d{1,3}[.-]?episode[.-]?\d{1,3}[-.]episode[.-]?\d{1,3}',
                r'saison[.-]?\d{1,3}[.-]?episode[.-]?\d{1,3}[-.]episode[.-]?\d{1,3}')
...
def remove_lang(release_info, check_foreign_audio):
    if not release_info:
        return False
    try:
        if any(value in release_info for value in DUBBED):
            return True
        if any(value in release_info for value in SUBS):
            return True
        if check_foreign_audio:
            if any(value in release_info for value in LANG) and not any(value in release_info for value in ENG_CHECK):
                return True

            if any(value in release_info for value in ABV_LANG) and not any(value in release_info for value in ENG_CHECK):
                return True

        if release_info.endswith('.srt.') and not any(value in release_info for value in SRT_CHECK):
            return True
...
def filter_season_pack(show_title, aliases, year, season, release_title):
    ...
    try:
        ...
        show_title = show_title.replace('!', '').replace('(', '').replace(')', '').replace('&', 'and').replace(year,
                                                                                                               '')  # year only in meta title if an addon custom query added it
        ...
        split_list = (
        season_full_check_fr, season_full_fill_check_fr, season_check, season_fill_check, season_fill_checke,
        '.' + season + '.season', 'total.season', 'season', 'the.complete', 'complete')

        if year or len(year) == 4:
            tmp = list(split_list)
            tmp.append('%s' % year)
            split_list = tuple(tmp)

        string_list = (
        season_full_fill_check_fr, season_full_check_fr, season_check, season_fill_check, season_fill_checke,
        season_full_check, season_full_check_ns, season_full_fill_check, season_full_fill_check_ns)
        ...
        episode_regex = (
            ...
            r'saison[.-]?\d{1,3}[.-]?ep[.-]?\d{1,3}[-.](?!\d{2,3}[-.])(?!e\d{1,3})(?!\d{2}gb)',
            r'season[.-]?\d{1,3}[.-]?episode[.-]?\d{1,3}[-.](?!\d{2,3}[-.])(?!e\d{1,3})(?!\d{2}gb)',
            r'saison[.-]?\d{1,3}[.-]?episode[.-]?\d{1,3}[-.](?!\d{2,3}[-.])(?!e\d{1,3})(?!\d{2}gb)')
        ...
        range_regex = (
            ...
            r'saison[.-]?\d{1,3}[.-]?ep[.-]?(\d{1,3})[-.]ep[.-]?(\d{1,3})',
            r'season[.-]?\d{1,3}[.-]?episode[.-]?(\d{1,3})[-.]episode[.-]?(\d{1,3})',
            r'saison[.-]?\d{1,3}[.-]?episode[.-]?(\d{1,3})[-.]episode[.-]?(\d{1,3})')  # may need to add "to", "thru"
        ...
        if any(i in rt for i in string_list):
            for item in (
                    ...
                    # ".season.1.to.9.", ".season.1.to.39"
                    season_full_check.rstrip('.') + r'[.-]season[.-]([2-9]{1}|[1-3]{1}[0-9]{1})(?:[.-]|$)',
                    # ".season.1.season.9.", ".season.1.season.39"
                    season_full_check.rstrip('.') + r'[.-]saison[.-]([2-9]{1}|[1-3]{1}[0-9]{1})(?:[.-]|$)',
                    # ".saison.1.season.9.", ".saison.1.season.39"
                    season_full_check.rstrip('.') + r'[.-]([2-9]{1}|[1-3]{1}[0-9]{1})(?:[.-]|$)',
                    # "season.1.9.", "season.1.39.
                    season_full_check.rstrip('.') + r'[.-]\d{1}[.-]\d{1,2}(?:[.-]|$)',  # "season.1.9.09."
                    season_full_check.rstrip('.') + r'[.-]\d{3}[.-](?:19|20)[0-9]{2}(?:[.-]|$)',
                    # single season followed by 3 digit followed by 4 digit year ex."season.1.004.1971"
                    season_full_fill_check.rstrip('.') + r'[.-]\d{3}[.-]\d{3}(?:[.-]|$)',
                    # 2 digit season followed by 3 digit dash range ex."season.10.001-025."
                    season_full_fill_check.rstrip('.') + r'[.-]season[.-]\d{2}(?:[.-]|$)',
                    # 2 digit season followed by 2 digit season range ex."season.01-season.09."
                    season_full_fill_check.rstrip('.') + r'[.-]saison[.-]\d{2}(?:[.-]|$)'
            # 2 digit season followed by 2 digit season range ex."saison.01-saison.09."
            ):
...
def filter_show_pack(show_title, aliases, imdb, year, season, release_title, total_seasons):
    ...
    try:
        ...
        episode_regex = (
            ...
            r'saison[.-]?\d{1,3}[.-]?ep[.-]?\d{1,3}',
            r'season[.-]?\d{1,3}[.-]?episode[.-]?\d{1,3}',
            r'saison[.-]?\d{1,3}[.-]?episode[.-]?\d{1,3}')
        ...
        season_regex = (
            ...
            # "season.1.01.complete" when 2nd number matches the fiirst group with leading 0
            r'saison[.-]?([1-9]{1})[.-]0{1}\1[.-]?complete',
            r'season[.-]?([2-9]{1})[.-](?:[0-9]+)[.-]?complete',
            # "season.9.10.complete" when first number is >1 followed by 2 digit number
            r'saison[.-]?([2-9]{1})[.-](?:[0-9]+)[.-]?complete',
            r'season[.-]?\d{1,2}[.-]s\d{1,2}',  # season.02.s02
            r'saison[.-]?\d{1,2}[.-]s\d{1,2}',
            r'season[.-]?\d{1,2}[.-]complete',  # season.02.complete
            r'saison[.-]?\d{1,2}[.-]complete',
            r'season[.-]?\d{1,2}[.-]\d{3,4}p{0,1}',  # "season.02.1080p" and no seperator "season02.1080p"
            r'saison[.-]?\d{1,2}[.-]\d{3,4}p{0,1}',
            r'season[.-]?\d{1,2}[.-](?!thru|to|\d{1,2}[.-])',
            # "season.02." or "season.1" not followed by "to", "thru", or another single or 2 digit number then a dot(which would be a range)
            r'saison[.-]?\d{1,2}[.-](?!thru|to|\d{1,2}[.-])',
            r'season[.-]?\d{1,2}[.]?$',
            # end of line ex."season.1", "season.01", "season01" can also have trailing dot or end of line(dash would be a range)
            r'saison[.-]?\d{1,2}[.]?$',
            r'season[.-]?\d{1,2}[.-](?:19|20)[0-9]{2}',
            # single season followed by 4 digit year ex."season.1.1971", "season.01.1971", or "season01.1971"
            r'saison[.-]?\d{1,2}[.-](?:19|20)[0-9]{2}',
            r'season[.-]?\d{1,2}[.-]\d{3}[.-]{1,2}(?:19|20)[0-9]{2}',
            # single season followed by 3 digits then 4 digit year ex."season.1.004.1971" or "season.01.004.1971" (comic book format)
            r'saison[.-]?\d{1,2}[.-]\d{3}[.-]{1,2}(?:19|20)[0-9]{2}',
            r'(?<!thru)(?<!to)(?<!\d{2})[.-]s\d{2}[.-]complete',
            # ".s01.complete" not preceded by "thru", "to", or 2 digit number
            r'(?<!thru)(?<!to)(?<!s\d{2})[.-]s\d{2}(?![.-]thru)(?![.-]to)(?![.-]s\d{2})(?![.-]\d{2}[.-])'
        # .s02. not preceded by "thru", "to", or "s01". Not followed by ".thru", ".to", ".s02", "-s02", ".02.", or "-02."
        )
        ...
        season_count = 2
        while season_count <= int(total_seasons):
            to_season_ranges.append(start_season + '.to.%s' % '0' + str(season_count) if int(
                season_count) < 10 else start_season + '.to.%s' % str(season_count))
            season_count += 1
        ...
        season_count = 2
        while season_count <= int(total_seasons):
            to_season_ranges.append(start_season + '.to.s%s' % '0' + str(season_count) if int(
                season_count) < 10 else start_season + '.to.s%s' % str(season_count))
            season_count += 1
...
def info_from_name(release_title, title, year, hdlr=None, episode_title=None, season=None, pack=None):
    try:
        ...
        if hdlr:
            name_info = name_info.replace(hdlr.lower(), '')
        ...
        if pack:
            if pack == 'season':
                season_fill = season.zfill(2)
                str1_replace = (
                '.saison.%s' % season_fill, '.saison.%s' % season, '.s%s' % season, '.s%s' % season_fill,
                '.season.%s' % season, '.season%s' % season, '.season.%s' % season_fill, '.season%s' % season_fill,
                'complete')
                ...
                str2_replace = (
                'saison', 'saisons', '.all.seasons', 'seasons', 'season', 'the.complete', 'complete', 'all.torrent',
                'total.series', 'tv.series', 'series', 'edited', 's1', 's01')
...
def release_title_format(release_title):
    try:
        ...
        fmt = '.%s.' % re.sub(r'[^a-z0-9-~]+', '.', release_title).replace('.-.', '-').replace('-.', '-').replace('.-',
                                                                                                                  '-').replace(
            '--', '-')
        return fmt
    except:
...
def _size(siz):
    try:
        ...
        div = 1 if siz.lower().endswith(('gb', 'go', 'gib')) else 1024
        ...
        if dec_count == 2:
            siz = siz.replace('.', ',', 1)  # torrentproject2 likes to randomly use 2 decimals vs. a comma then a decimal
...
def is_host_valid(url, domains):
    try:
        if any(x in url.lower() for x in ('.rar.', '.zip.', '.part.', '.sample.')) or any(
                url.lower().endswith(x) for x in
                ('.bmp', '.gif', '.jpg', '.nfo', '.part', '.png', '.rar', '.sample.', '.srt', '.txt', '.zip')):
            return False, ''
