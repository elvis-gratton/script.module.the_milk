...
from the_milk.modules.control import existsPath, dataPath, makeFile, plexSharesFile
...
class Plex():
    ...
    def auth_loop(self):
        ...
        try:
            self.token = re.search(r'<auth_token>(.*?)</auth_token>', data.text, re.I).group(1)
        except:
            self.token = ''
        if self.token:
    ...
    def auth(self):
        ...
        control.progressDialog.update(-1, control.progress_line % (
        control.lang(32513) % 'https://plex.tv/link/', control.lang(32514) % code, ''))
    ...
    def get_plexshare_resource(self):
        url = base_url + '/api/v2/resources?includeHttps=1&X-Plex-Client-Identifier=%s&X-Plex-Token=%s' % (
    ...
    def revoke(self):
        ...
        if self.clearPlex() == True:
            from the_milk.modules import log_utils
            log_utils.log('the_milk Cleared Plex Database.', 1)
        else:
            from the_milk.modules import log_utils
            log_utils.log('the_milk Clear Plex Database Failed.', 1)
    ...
    def plex_get_all(self):
        try:
            ...
            dbcur = dbcon.cursor()
            dbcur.execute(
                '''CREATE TABLE IF NOT EXISTS plexshares (sourceTitle TEXT, accessToken TEXT, plexshareURL TEXT, UNIQUE(plexshareURL));''')
