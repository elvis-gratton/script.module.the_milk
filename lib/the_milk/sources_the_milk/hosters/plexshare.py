...
from the_milk.modules import cleantitle, source_utils
from the_milk.modules.client import parseDOM, replaceHTMLCodes
from the_milk.modules import source_utils
from the_milk.modules import workers
from the_milk.modules import control
from the_milk.modules import plex as plexShare
...
class source:
    ...
    def get_sources(self, sourceTitle, accessToken, plexShareURL, data):
        ...
        moviesearch = '/search?type=1&query=%s&year=%s&limit=100&X-Plex-Client-Identifier=%s&X-Plex-Token=%s' % (
        '%s', '%s', self.client_id, accessToken)
        episodesearch = '/hubs/search?query=%s&limit=30&X-Plex-Client-Identifier=%s&X-Plex-Token=%s' % (
        '%s', self.client_id, accessToken)
        if self.composite_installed:
            play_link = 'plugin://plugin.video.composite_for_plex/?url=%s' % base_link
        else:
            play_link = base_link
        ...
        for result in results:
            try:
                ...
                if cleantitle.get(title.replace('&', 'and')) not in cleantitle.get(
                        name.replace('&', 'and')):  # use meta title in case file naming is whacked
                    if not episode_title:
                        meta_title = cleantitle.normalize(
                            replaceHTMLCodes(parseDOM(result, 'Video', ret='title')[0].replace(' ', '.')))
                        meta_year = parseDOM(result, 'Video', ret='year')[0]
                        name = '%s.%s' % (meta_title, meta_year)
                    else:
                        meta_title = cleantitle.normalize(replaceHTMLCodes(
                            parseDOM(result, 'Video', ret='grandparentTitle')[0].replace('&amp;', '&').replace(' ',
                                                                                                               '.')))
                        meta_season, meta_episode = parseDOM(result, 'Video', ret='parentIndex')[0], \
                        parseDOM(result, 'Video', ret='index')[0]
                ...
                if not name_info or len(
                        name_info) <= 5:  # use meta info because file name lacks info, or has video extension only
                ...
                if self.composite_installed:
                    ...
                    url = '%s%s?X-Plex-Client-Identifier=%s&X-Plex-Token=%s&mode=5' % (
                    play_link, key, self.client_id, accessToken)
                else:
                    key = parseDOM(result, 'Part', ret='key')[0].rsplit('/', 1)[
                        0]  # remove "/file.mkv" to replace with true file name for dnld
                    url = '%s%s/%s?X-Plex-Client-Identifier=%s&X-Plex-Token=%s' % (
                    play_link, key, file_name, self.client_id, accessToken)
                ...
                info = ' | '.join(info)
                self.sources_append(
                    {'provider': 'plexshare', 'plexsource': sourceTitle, 'source': 'direct', 'name': file_name,
                     'name_info': name_info,
                     'quality': quality, 'language': 'en', 'url': url, 'info': info, 'direct': True,
                     'debridonly': False, 'size': dsize})
            except:
                source_utils.scraper_error('PLEXSHARE')
