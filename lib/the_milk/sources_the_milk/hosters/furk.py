# -*- coding: UTF-8 -*-
# (updated 7-19-2022) increased timeout=20
'''
...
import requests
from the_milk.modules.control import setting as getSetting, setSetting
...
class source:
    ...
    def __init__(self):
        ...
        self.search_link = "/api/plugins/metasearch?api_key=%s&q=%s&cached=yes&match=%s&moderated=%s%s&sort=relevance&type=video&offset=0&limit=200"
    ...
    def sources(self, data, host_dict):
        ...
        try:
            ...
            if 'tvshowtitle' in data:
                ...
                query = '@name+%s+@files+%s+|+%s+|+%s+|+%s+|+%s' % (
                title, seasEpList[0], seasEpList[1], seasEpList[2], seasEpList[3], seasEpList[4])
            else:
        ...
        for i in files:
            try:
                if i['is_ready'] == '1' and i['type'] == 'video':
                    ...
                    append(
                        {'provider': 'furk', 'source': source, 'name': name, 'name_info': name_info, 'quality': quality,
                         'language': "en", 'url': url,
    ...
    def resolve(self, url):
        try:
            ...
            if self.content_type == 'episode': self.filtering_list = self._seas_ep_resolve_list(url.get('season'),
                                                                                                url.get('episode'))
