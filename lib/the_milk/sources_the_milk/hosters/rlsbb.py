...
class source:
    ...
    def __init__(self):
        self.language = ['en']
        self.base_new = "http://proxybb.com/"  # cloudflare captcha
        # self.base_new = "https://releasebb.net/"
    ...
    def sources(self, data, host_dict):
        ...
        try:
            ...
            if int(year) >= 2021:
                self.base_link = self.base_new
            # else: self.base_link = self.base_old
            else:
                return sources  # "old3.proxybb.com" does not seem operational anymore

            url = '%s%s' % (self.base_link, query)
            # log_utils.log('url = %s' % url, log_utils.LOGDEBUG)
            r = scraper.get(url, headers=self.headers, timeout=5).text
        ...
        for post in posts:
            ...
            try:
                post_titles = re.findall(r'(?:.*>|>\sRelease Name.*|\s)(%s.*?)<' % release_title, post,
                                         re.I)  # parse all matching release_titles in each post(content) group
                if len(post_titles) > 1:
                    ...
                    for name in post_titles:
                        start = post_titles[index].replace('[', '\\[').replace('(', '\\(').replace(')', '\\)').replace(
                            ...
                            post_titles[index + 1].replace('[', '\\[').replace('(', '\\(').replace(')', '\\)').replace(
                                '+', '\\+')).replace(' \\ ', ' \\\\ ') if index + 1 < len(post_titles) else ''
                        try:
                            container = re.findall(r'(?:%s)([\S\s]+)(?:%s)' % (start, end), post, re.I)[
                                0]  # parse all data between release_titles in multi post(content) group
                        except:
                            source_utils.scraper_error('RLSBB')
                            continue
                        try:
                            size = re.search(r'((?:\d+\,\d+\.\d+|\d+\.\d+|\d+\,\d+|\d+)\s*(?:GB|GiB|Gb|MB|MiB|Mb))',
                                             container).group(0).replace(',', '.')
                        except:
                            size = '0'
                    ...
                    name = post_titles[0]
                    container = client.parseDOM(post, 'a',
                                                ret='href')  # parse all links in a single post(content) group
                    try:
                        size = re.search(r'((?:\d+\,\d+\.\d+|\d+\.\d+|\d+\,\d+|\d+)\s*(?:GB|GiB|Gb|MB|MiB|Mb))',
                                         post).group(0).replace(',', '.')
                    except:
                        size = '0'
                    items_append((name, size, container))
                else:
                ...
                for group_name, size, links in items:
                    for i in links:
                        ...
                        append({'provider': 'rlsbb', 'source': host, 'name': name, 'name_info': name_info,
