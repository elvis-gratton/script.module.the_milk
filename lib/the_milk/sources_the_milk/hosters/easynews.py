...
SEARCH_PARAMS = {'st': 'adv', 'sb': 1, 'fex': 'm4v,3gp,mov,divx,xvid,wmv,avi,mpg,mpeg,mp4,mkv,avc,flv,webm',
...
class source:
    ...
    def sources(self, data, host_dict):
        ...
        try:
            ...
            if 'tvshowtitle' in data:
                ...
                hdlr = year
                query = '%s' % re.sub(r'[^A-Za-z0-9\s\.-]+', '',
                                      title)  # let "source_utils.check_title()" verify "release_title" contains a matching year in years list
        ...
        for item in files:
            try:
                ...
                sources_append({'provider': 'easynews', 'source': 'direct', 'name': name, 'name_info': name_info,
                                'quality': quality,
                                'language': "en", 'url': file_dl, 'info': info, 'direct': True, 'debridonly': False,
                                'size': dsize})
            except:
                source_utils.scraper_error('EASYNEWS')
        return sources
    ...
    def _translate_search(self, query):
        ...
        params['safeO'] = 1 if getSetting(
            'easynews.moderation') == 'true' else 0  # 1 is the moderation(adult filter) ON, 0 is OFF.
