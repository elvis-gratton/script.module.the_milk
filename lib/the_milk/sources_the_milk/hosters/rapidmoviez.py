...
from the_milk.modules import dom_parser  # switch to client.parseDOM() to rid import
...
class source:
    ...
    def sources(self, data, host_dict):
        ...
        try:
            ...
            int(data['season']), int(data['episode'])) if 'tvshowtitle' in data else self.year
            imdb = data['imdb']
    ...
    def get_sources(self, name, url):
        try:
            ...
            for link in urls:
                ...
                self.sources_append({'provider': 'rapidmoviez', 'source': host, 'name': name, 'name_info': name_info,
