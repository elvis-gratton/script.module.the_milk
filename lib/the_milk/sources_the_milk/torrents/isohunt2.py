...
_DATA = re.compile(
    r'<a\s*href\s*=\s*["\'](/torrent_details/.+?)["\']><span>(.+?)</span>.*?<td\s*class\s*=\s*["\']size-row["\']>(.+?)</td><td\s*class\s*=\s*["\']sn["\']>([0-9]+)</td>',
    re.I)
    ...
    def get_sources(self, row):
        ...
        for items in data:
            try:
                ...
                try:
                    url = \
                    unquote_plus(re.search(r'(magnet.*?)["\']', result).group(1)).replace('&amp;', '&').split('&tr')[
                        0].replace(' ', '.')
                except:
                    continue
                ...
                self.sources_append(
                    {'provider': 'isohunt2', 'source': 'torrent', 'seeders': seeders, 'hash': hash, 'name': name,
                     'name_info': name_info,
