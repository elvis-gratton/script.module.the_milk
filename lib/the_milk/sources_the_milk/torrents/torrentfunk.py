...
class source:
    ...
    def get_sources(self, link):
        try:
            ...
            try:
                size = re.search(r'Size:.*?>((?:\d+\,\d+\.\d+|\d+\.\d+|\d+\,\d+|\d+)\s*(?:GB|GiB|Gb|MB|MiB|Mb))',
                                 link).group(1)
            ...
            self.sources_append(
                {'provider': 'torrentfunk', 'source': 'torrent', 'seeders': seeders, 'hash': hash, 'name': name,
                 'name_info': name_info,
            ...
            try:
                ...
                if not self.search_series:
                    if not self.bypass_filter:
                        valid, self.episode_start, self.episode_end = source_utils.filter_season_pack(self.title,
                                                                                                      self.aliases,
                                                                                                      self.year,
                                                                                                      self.season_x,
                                                                                                      name)
                ...
                if self.search_series:
                    self.items.append((name, name_info, url, package, last_season))
                else:
                    self.items_append((name, name_info, url, package))
            except:
                source_utils.scraper_error('TORRENTFUNK')
    ...
    def get_pack_sources(self, items):
        try:
            ...
            item = {'provider': 'torrentfunk', 'source': 'torrent', 'seeders': seeders, 'hash': hash, 'name': items[0],
                    'name_info': items[1], 'quality': quality,
                    'language': 'en', 'url': url, 'info': info, 'direct': False, 'debridonly': True, 'size': dsize,
                    'package': items[3]}
            if self.search_series:
                item.update({'last_season': items[4]})
            elif self.episode_start:
                item.update(
                    {'episode_start': self.episode_start, 'episode_end': self.episode_end})  # for partial season packs
