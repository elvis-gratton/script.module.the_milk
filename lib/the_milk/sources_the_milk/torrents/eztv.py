        ...
        for row in rows:
            try:
                ...
                try:
                    ...
                    link = \
                    re.findall(r'href\s*=\s*["\'](magnet:[^"\']+)["\'].*?title\s*=\s*["\'](.+?)["\']', columns[2],
                               re.DOTALL | re.I)[0]
                except:
                    continue
                ...
                sources_append({'provider': 'eztv', 'source': 'torrent', 'seeders': seeders, 'hash': hash, 'name': name,
                                'name_info': name_info,
        ...
        for row in rows:
            try:
                ...
                if not bypass_filter:
                    valid, episode_start, episode_end = source_utils.filter_season_pack(title, aliases, year, season_x,
                                                                                        name)
                ...
                item = {'provider': 'eztv', 'source': 'torrent', 'seeders': seeders, 'hash': hash, 'name': name,
