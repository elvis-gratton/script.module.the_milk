...
_DATA = re.compile(
    r'<a\s*href\s*=\s*["\'](/torrent/.+?/)["\']\s*title\s*=\s*["\'](.+?)["\'].*?<td\s*class\s*=\s*["\']digits["\']>(.*?)<.*?<td\s*class\s*=["\']digits["\']\s*data-title\s*=\s*["\']S["\']>(.*?)</td>',
    re.I)
    ...
    def get_sources(self, row):
        ...
        for items in data:
            try:
                ...
                try:
                    url = unquote_plus(
                        re.search(r'(magnet:.*?)["\']', result).group(1).replace('\\x26', '&').replace('\\x2b',
                                                                                                       '+').replace(
                            'X-X', '')).split('&tr=')[0]
                except:
                    continue
                ...
                self.sources_append(
                    {'provider': 'ibit', 'source': 'torrent', 'seeders': seeders, 'hash': hash, 'name': name,
                     'name_info': name_info,
