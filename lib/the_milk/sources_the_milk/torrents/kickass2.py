    ...
    def __get_base_url(self, fallback):
        for domain in self.domains:
            try:
                ...
                try:
                    result = re.search('r<title>(.+?)</title>', result, re.I).group(1)
                except:
                    result = None
    ...
    def get_sources(self, url):
        ...
        for row in rows:
            try:
                ...
                info = ' | '.join(info)
                self.sources_append(
                    {'provider': 'kickass2', 'source': 'torrent', 'seeders': seeders, 'hash': hash, 'name': name,
                     'name_info': name_info,
        ...
        for row in rows:
            try:
                ...
                try:
                    ...
                    dsize = 0
                info = ' | '.join(info)
                item = {'provider': 'kickass2', 'source': 'torrent', 'seeders': seeders, 'hash': hash, 'name': name,
