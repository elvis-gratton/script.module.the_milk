...
class source:
    ...
    def get_sources(self, link):
        try:
            ...
            name = re.search(r'<h3\s*class\s*=\s*["\']card-title["\']>(.+?)<', result, re.I).group(1).replace(
                'Original Name: ', '')
            ...
            try:
                seeders = int(
                    re.search(r'>Seeders:.*?>\s*([0-9]+|[0-9]+,[0-9]+)\s*</', result, re.I).group(1).replace(',', ''))
            ...
            self.sources_append(
                {'provider': 'yourbittorrent', 'source': 'torrent', 'seeders': seeders, 'hash': hash, 'name': name,
                 'name_info': name_info,
    ...
    def get_pack_sources(self, url):
        try:
            ...
            item = {'provider': 'yourbittorrent', 'source': 'torrent', 'seeders': seeders, 'hash': hash, 'name': name,
