    ...
    def sources(self, data, host_dict):
        ...
        for row in rows:
            try:
                ...
                append({'provider': 'glodls', 'source': 'torrent', 'seeders': seeders, 'hash': hash, 'name': name,
                        'name_info': name_info,
    ...
    def get_sources_packs(self, link):
        ...
        for row in rows:
            try:
                ...
                hash = re.search(r'btih:(.*?)&', url, re.I).group(1)
                name = client.parseDOM(columns[1], 'a', ret='title')[0].replace('/', '').replace('  ',
                                                                                                 ' ')  # glotorrents seems to fix incomplete packs as a range in html title tag
                ...
                item = {'provider': 'glodls', 'source': 'torrent', 'seeders': seeders, 'hash': hash, 'name': name,
