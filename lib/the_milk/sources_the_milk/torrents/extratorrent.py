...
# from the_milk.modules import cfscrape # proxy in use not behind CloudFlare
    ...
    def get_sources(self, url):
        ...
        for row in rows:
            try:
                ...
                url = unquote_plus(url).replace('&amp;', '&').replace('&amp;', '&').replace(' ', '.').split('&tr')[
                    0]  # some links on extratorrent dbl "&amp;amp;"
                ...
                self.sources_append(
                    {'provider': 'extratorrent', 'source': 'torrent', 'seeders': seeders, 'hash': hash, 'name': name,
                     'name_info': name_info,
    ...
    def get_sources_packs(self, link):
        ...
        for row in rows:
            try:
                ...
                item = {'provider': 'extratorrent', 'source': 'torrent', 'seeders': seeders, 'hash': hash, 'name': name,
