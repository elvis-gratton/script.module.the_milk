    ...
    def sources(self, data, host_dict):
        ...
        for file in files:
            try:
                ...
                sources_append(
                    {'provider': 'torrentio', 'source': 'torrent', 'seeders': seeders, 'hash': hash, 'name': name,
                     'name_info': name_info,
            ...
            try:
                ...
                if not search_series:
                    if not bypass_filter:
                        valid, episode_start, episode_end = source_utils.filter_season_pack(title, aliases, year,
                                                                                            season, name.replace(
                                '.(Archie.Bunker', ''))
                    ...
                    if not bypass_filter:
                        valid, last_season = source_utils.filter_show_pack(title, aliases, imdb, year, season,
                                                                           name.replace('.(Archie.Bunker', ''),
                                                                           total_seasons)
                        if not valid: continue
                    else:
                ...
                item = {'provider': 'torrentio', 'source': 'torrent', 'seeders': seeders, 'hash': hash, 'name': name,
