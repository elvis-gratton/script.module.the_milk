    ...
    def sources(self, data, host_dict):
        ...
        for file in files.get('content'):
            try:
                ...
                try:
                    size = file.get('size')
                    size = str(size) + ' GB' if len(str(size)) <= 2 else str(
                        size) + ' MB'  # bitlord size is all over the place between MB and GB
                ...
                append({'provider': 'bitlord', 'source': 'torrent', 'seeders': seeders, 'hash': hash, 'name': name,
                        'name_info': name_info,
    ...
    def get_sources_packs(self, link, url):
        ...
        for file in files.get('content'):
            try:
                ...
                item = {'provider': 'bitlord', 'source': 'torrent', 'seeders': seeders, 'hash': hash, 'name': name,
